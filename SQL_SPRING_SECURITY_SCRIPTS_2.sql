...
SELECT `account_transactions`.`transaction_id`,
    `account_transactions`.`account_number`,
    `account_transactions`.`customer_id`,
    `account_transactions`.`transaction_dt`,
    `account_transactions`.`transaction_summary`,
    `account_transactions`.`transaction_type`,
    `account_transactions`.`transaction_amt`,
    `account_transactions`.`closing_balance`,
    `account_transactions`.`create_dt`
FROM `eazy_bank`.`account_transactions`;

SELECT `contact_messages`.`contact_id`,
    `contact_messages`.`contact_name`,
    `contact_messages`.`contact_email`,
    `contact_messages`.`subject`,
    `contact_messages`.`message`,
    `contact_messages`.`create_dt`
FROM `eazy_bank`.`contact_messages`;
...
SELECT `cards`.`card_id`,
    ...
    `cards`.`total_limit`,
    `cards`.`amount_used`,
    `cards`.`available_amount`,
    `cards`.`create_dt`
FROM `eazy_bank`.`cards`;
...
SELECT `loans`.`loan_number`,
    ...
    `loans`.`create_dt`
FROM `eazy_bank`.`loans`;

SELECT `notice_details`.`notice_id`,
    `notice_details`.`notice_summary`,
    `notice_details`.`notice_details`,
    `notice_details`.`notic_beg_dt`,
    `notice_details`.`notic_end_dt`,
    `notice_details`.`create_dt`,
    `notice_details`.`update_dt`
FROM `eazy_bank`.`notice_details`;
